...
def to_placement_dict(placement: BowlPlacement, target_name: str | None = None,
                      species_human=None) -> dict:
    """Return the neuromod ``placement.json`` dict.

    ``species_human`` is a tuba species module (injected for testability); if None we
    import ``tuba.species.human`` lazily. The OUTPUT FRAME depends on it:
      * species available  -> coordinates in NRRD-voxel-mm  (``frame='nrrd_voxel_mm'``)
      * tuba unavailable   -> coordinates in MNI RAS mm      (``frame='mni_ras_mm'``)
    ...
    if species_human is not None:
        ...
        win, targ, frame = win_mni, targ_mni, "mni_ras_mm"
