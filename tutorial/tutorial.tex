...
\IfFileExists{../manuscript/figs/movie_frames/frame-0.png}{%
\animategraphics[controls,loop,width=0.82\linewidth]{12}{../manuscript/figs/movie_frames/frame-}{0}{134}%
}{%
\includegraphics[width=0.82\linewidth]{figs/fig_propagation_frame.png}%
}
...
\begin{figure}[H]\centering
\includegraphics[width=0.82\linewidth]{figs/targets_in_skull.png}
...
\end{lstlisting}
For a transparency/placement run the outward solve can record \emph{only the calvarial
surface}: \code{--recorder shell} replaces the decimated full-volume dump with the
surface shell (all the transparency map needs), keeping the output under a gigabyte --
this is what the Colab notebook and the precomputed gallery use. The default
(\code{--recorder volume}) also records the decimated interior field, needed e.g.\ for
focal-spot analysis. The solver runs at CFL $0.2$; the outward record length is
...
($\cos^2\theta$ on the true surface normal) and hard-rejects patches beyond the acceptance
angle (\code{theta_max_deg}, default $35^\circ$; the water-to-bone longitudinal critical
angle sits near $30^\circ$ -- the manuscript's analysis value -- past which the fluid
solver's longitudinal transmission gives way to unmodeled shear conversion); the resulting incidence-weighted
...
\begin{figure}[H]\centering
\includegraphics[width=0.72\linewidth]{figs/focus_energy_qc.png}
...
approach.
\item \textbf{Targeted \code{prepare} sizes a cube.} A targeted run uses an $N^3$ grid
from the bowl reach plus a margin, which is correct but spends more voxels than the
hand-cropped reference run. (Brain-center runs, \code{--center}, are tighter: the grid is
sized to the head's own bounding box, shrinkable further with \code{--surround-mm} and the
spine/mandible-trimming \code{--truncate-mm}.)
\end{itemize}
...
\end{lstlisting}
A richer manual tool (Appendix A of the PMB manuscript; a research prototype that is
not distributed with this repository -- the packaged \code{position --interactive} viewer
is its library form) renders the skull coloured by the placement objective $\sqrt{J_w}$
with a translucent CTX-500 cap and three orthogonal slices, with a live score as a
percentage of the global peak (Figure~\ref{fig:tool}).

Two further console commands consume the same bundle. \code{skull-transparency report
--bundle run/bundle --out report.html} writes a self-contained HTML placement report (the
transparency and placement-objective maps unwrapped over the skull, the chosen window with
ranked alternatives, and the coordinates/incidence/EEG-site table; print it to PDF for
archival). \code{skull-transparency explore} opens a map interactively -- by bundle path,
or by named target from the precomputed gallery (\code{--list-targets}).

\begin{figure}[H]\centering
\includegraphics[width=\linewidth]{figs/positioning_tool.png}
