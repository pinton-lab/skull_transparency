...
transducer placement plus a coupling score.
...
arrival. Sampling $p_{\max}$ just outside the bone surface and forming the peak intensity
$I=p_{\max}^2/(2\rho c)$, distance-corrected for $1/r^2$ spreading, gives the coupling
(transparency) map the placement search reads. That search weights each patch by incidence
...
\code{--approach} unit vector (target\,$\to$\,skin).
...
Two entry points need different things. If you already have a solved bundle you never
touch a CT, a GPU, or MATLAB, so install the core and go straight to placement
(Section~\ref{sec:steps}, step~5). If you are bringing your own CT you also need the
solver extra and a GPU.
