...
For a transparency/placement run the outward solve can record *only the calvarial surface*: `--recorder shell` replaces the decimated full-volume dump with the surface shell (all the transparency map needs), keeping the output under a gigabyte -- this is what the Colab notebook and the precomputed gallery use. The default (`--recorder volume`) also records the decimated interior field, needed e.g. for focal-spot analysis. The solver runs at CFL $0.2$; the outward record length is $\approx 1.84\,\lvert\mathbf{x}_\mathrm{array}-\mathbf{x}_t\rvert_{\max}/c_0$. Each phase writes `genout.dat` (the recorded channels and frames). The medium maps depend only on $(c,\text{grid})$, so the inward run hardlinks them from the outward run instead of rewriting $\sim$10 GB.
...
- **Targeted `prepare` sizes a cube.** A targeted run uses an $N^3$ grid from the bowl reach plus a margin, which is correct but spends more voxels than the hand-cropped reference run. (Brain-center runs, `--center`, are tighter: the grid is sized to the head's own bounding box, shrinkable further with `--surround-mm` and the spine/mandible-trimming `--truncate-mm`.)
...
A richer manual tool (Appendix A of the PMB manuscript; a research prototype that is not distributed with this repository -- the packaged `position --interactive` viewer is its library form) renders the skull coloured by the placement objective $\sqrt{J_w}$ with a translucent CTX-500 cap and three orthogonal slices, with a live score as a percentage of the global peak (Figure 8).

Two further console commands consume the same bundle. `skull-transparency report --bundle run/bundle --out report.html` writes a self-contained HTML placement report (the transparency and placement-objective maps unwrapped over the skull, the chosen window with ranked alternatives, and the coordinates/incidence/EEG-site table; print it to PDF for archival). `skull-transparency explore` opens a map interactively -- by bundle path, or by named target from the precomputed gallery (`--list-targets`).
